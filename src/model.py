...
class Model():
    ...
    def __create_placeholders__(self):
        self.x_root = tf.placeholder(
            dtype=tf.float32,
            shape=[config.batch_size, config.seq_length, 1],
            name='input_x_root'
        )
        self.x_neighbour = tf.placeholder(
            dtype=tf.float32,
            shape=[config.batch_size, config.seq_length, config.num_neighbour],
            name='input_x_neighbour'
        )
        self.decode_seqs = tf.placeholder(
            dtype=tf.float32,
            shape=[config.batch_size, config.seq_length + 1, 1], # start_id at beginning
            name="decode_root_seqs"
        )
        self.target_seqs = tf.placeholder(
            dtype=tf.float32,
            shape=[config.batch_size, config.seq_length + 1, 1], # end_id at end
    ...
    def __get_network__(self, x_root, x_neighbour, decode_seqs, is_train=True, reuse=False):
        ...
        with tf.variable_scope(self.model_name, reuse=reuse) as vs:
            ...
            inputs_x_root = InputLayer(x_root, name='in_root')
            inputs_x_nbor = InputLayer(x_neighbour, name="in_neighbour")

            # encoding neighbour graph information
            n = ReshapeLayer(inputs_x_nbor, (config.batch_size * config.seq_length, config.num_neighbour), "reshape1")
            ...
            n = ReshapeLayer(n, (config.batch_size, config.seq_length, -1), name="reshape1_back")

            net_encode = ConcatLayer([inputs_x_root, n], concat_dim=-1, name="encode")
            net_decode = InputLayer(decode_seqs, name="decode")
            ...
            net_out = DenseLayer(net_rnn, n_units=1, act=tf.identity, name='dense')
            net_out = ReshapeLayer(net_out, (config.batch_size, config.seq_length + 1, 1), name="reshape_out")
            return net_out
